        ...
        for j=1:size(c,2)
            tau_temp = cell2mat(tau(1,i));
             if (c(i,j)~=0  && (round(tau_temp(j,t-1)*365) > 0))
                ...
                sum = sum - AE_temp(round(tau_temp(j,t-1)*365),:)*AE_temp(round(tau_temp(j,t-1)*365),:)'*tau_temp(j,t)^2*c(i,j)*exp(-spot_rate_temp(t-1,round(tau_temp(j,t-1)*365))*tau_temp(j,t));
             end
        end
        hessian(:,:,i) = sum;
        sum = zeros(6,6);
