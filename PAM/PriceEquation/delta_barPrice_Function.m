...
%theoretical_price_yesterday =  create_theoretical_price_xi_tilde_yesterday(prevC,prevTau,spot_rates,N,t,prevCurrency,Nc,prevT_cashflow); %oklart om detta är rätt
...
for i = 1:N
    ...
    %disp(size(tau_temp))
    %disp(tau_temp)
    for j = 1:size(prevC,2) %HÄR ÄNDRADE Isak från size(c,1) 7/12
        %disp(j)
