function [f,z,f_given, T] = IRC(day)
    ...
    T = [90 180 270 1*365 2*365 3*365 4*365 5*365 6*365 7*365 ...
         8*365 9*365 10*365 11*365 12*365 13*365 14*365 15*365 20*365];
    n_r = length(T);
    n_f = T(end);
    discountFactors = discountFactors(day, 7:n_r+6);
    forwardRates = forwardRates(day, 7:n_r+6);
    b = -forwardRates';
    deltaT = 1/365;
