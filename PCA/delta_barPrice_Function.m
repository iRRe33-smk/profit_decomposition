...
[risk_factors, gradient_price, hessian_price ,spot_rates] = get_PCA_data(c, tau,forward_rates);
c = [c;c2];
tau = [tau,tau2];
% N= N assets
%T = t timesteps
...
for t = 1:T
    for i = 1:N
    delta_price_xi(i,t) = diag(delta_xi(t))*gradient_price(:,t); % gradient from PCA.
    end
end
...
%(513)
theoretical_price_xi_tilde = create_theoretical_price_xi_tilde(c,tau,spot_rates);
%theoretical_price_s = 
 delta_epsilon_i = theoretical_price_xi_tilde-theoretical_price_s; % två separata funktioner på matlab (kassaflödessumma).
...
delta_barPrice = delta_price_a + delta_epsilon_p + delta_epsilon_a + delta_epsilon_i; %dP = [nP,nC]
