...
function setUp()
  ...
  % file names
  javaapiname = 'ampl-2.0.3.0.jar';
  matlabbridgename = 'matlab-lib.jar'
  if ispc
    binlibraryname = fullfile('bin', 'ampl-2.0.3.0.dll');
    binbase = fullfile(base, '..', 'bin');
  elseif ismac
    binlibraryname = fullfile('lib', 'libampl.2.0.3.0.dylib');
    binbase = fullfile(base, '..', 'lib');
  else
    binlibraryname = fullfile('lib', 'libampl.so.2.0.3.0');
    binbase = fullfile(base, '..', 'lib');
  end
